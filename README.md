...
安装包通过 GitHub Release 分发，不建议把 `.exe` 直接提交进 git。
...
请确认使用的是最新版本。当前版本清除配置时会删除新旧配置文件，并清空后端运行时状态。旧版本可能只删了一个配置目录，导致旧开发配置被重新读取。
...
- 不要提交真实 API Key、访问令牌或敏感会议日志。
- `.env` 文件不应进入版本控制。
- 发布安装包时，API Key 不会被打包进应用。每个用户需要在自己的应用设置中配置自己的 key。
- 如果用于团队或公开演示，建议使用权限受限的专用 DashScope API Key。
