...
class ConnectionManager:
    ...
    def update_settings(self, websocket: WebSocket, new_settings: Dict):
        if websocket in self.connection_settings:
            self.connection_settings[websocket].update(new_settings)
    ...
    try:
        while True:
            ...
            if data["type"] == "websocket.receive":
                if "bytes" in data:
                    await handle_audio_data(websocket, data["bytes"])
                elif "text" in data:
                    await handle_text_message(websocket, data["text"])
...
async def handle_audio_data(websocket: WebSocket, audio_data: bytes):
    ...
    translated_text = translate_result.get("translated", "")
...
async def handle_text_message(websocket: WebSocket, message: str):
    try:
